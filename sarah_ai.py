...
def play1(*q):								#To play a particular song
	...
	if len(same)==1:
		...
		q="'"+q+"'"
		playing.append(q)
		selected='cvlc --play-and-exit /root/rakesh-chinta/Music/'+q
		os.system(selected)
		playing.remove(q)
		...
		if l[1]=='stop':						#to stop the playing music
			if l[2]=='song' or l[2]=='music':
				if(len(playing)>0):
					...
					speak('Not playing anything sir')
		if l[1]=='what':
...
speak('Sarah at your command sir')
